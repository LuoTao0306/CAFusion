...
from losses import ssim_spect, ssim_mri,RMI_spect,RMI_mri,MP_AG_loss,MP_EN_loss,ThresholdBasedLoss
device = torch.device('cuda:0')
use_gpu = torch.cuda.is_available()
# from losses import ssim_ir, ssim_vi,RMI_ir,RMI_vi
def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--name', default="ablation_NO_detailfattention", help='model name: (default: arch+timestamp)')
...
def train(args, train_loader, model, criterion_ssim_spect, criterion_ssim_mri,criterion_RMI_spect,criterion_RMI_mri,optimizer, epoch, scheduler=None):
    ...
    loss_Fre =AverageMeter()
    weight = args.weight#默认使用自适应损失函数，使用手动时，需要设置
    model.train()
    #开始训练模型
    for i, (input,spect,mri)  in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        #频域loss
        # print(mri.shape)
        # T = TriangleThresholdModel()
        # loss_fre_all = T(mri,spect,SPECT)
        mloss = ThresholdBasedLoss()
        loss_fre_all = mloss(mri,spect,out)



        loss = loss_ssim_spect + loss_ssim_mri+loss_RMI_spect+ loss_RMI_mri + loss_fre_all
        ...
        loss_Fre.update(loss_fre_all.item(), input.size(0))
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
class my_GetDataset(Dataset):
    def __init__(self, spect_imageFolderDataset, mri_imageFolderDataset,transform=None):
        """
        :param spect_imageFolderDataset: MRI数据集
        :param mri_imageFolderDataset: SPECT数据集
        :param transform:
        """
        self.spect_imageFolderDataset = spect_imageFolderDataset
        self.mri_imageFolderDataset = mri_imageFolderDataset
...
def main():
    ...
    training_dir_spect =r"D:\coding\pycharm_projects\paper3\twoDomainModelMultiBatchsize\datasets\SPECT-MRI\train_data\SPECT\\*"
    folder_dataset_train_spect =sorted(glob.glob(training_dir_spect))
    training_dir_mri =r"D:\coding\pycharm_projects\paper3\twoDomainModelMultiBatchsize\datasets\SPECT-MRI\train_data\MRI\\*"
    ...
    train_loader = DataLoader(my_datase, shuffle=True, num_workers=4,batch_size=8)
    # dim = Image.open(folder_dataset_train_spect[0]).convert('L').size[0]
    ...
    for epoch in range(args.epochs):
        ...
        print('loss: %.4f - loss_ssim_spect: %.4f - loss_ssim_mri: %.4f - loss_RMI_spect: %.4f - loss_RMI_mri: %.4f - loss_Fre: %.4f '
              % (train_log['loss'],
        ...
        ], index=['epoch', 'loss', 'loss  _ssim_spect', 'loss_ssim_mri', 'loss_RMI_spect', 'loss_RMI_mri', 'loss_Fre'])
